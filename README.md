...
- **F17 / F18:** hold to send joystick up / down through the HMD input profile while the dashboard is open. This adds no virtual hand. Release either key to return the axis to neutral. The user confirmed the control works in the headset.
- **F19 (prototype):** toggle the dashboard pointer between head aim and eye gaze. Invalid or stale gaze falls back to head aim. F14 click and drag and the existing game eye-tracking feed remain separate. The gaze pointer still needs a headset test.
