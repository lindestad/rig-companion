...
- **F17 / F18 (prototype):** hold to send joystick up / down through the HMD input profile while the dashboard is open. This adds no virtual hand. SteamVR binds the input to dashboard scrolling; whether it moves a grabbed floating desktop view in depth still needs a headset test. Release either key to return the axis to neutral.
